// LiveBall Javascript API (intended for use on LiveBall pages)
//
// Copyright (c) 2009 ion interactive, inc.   All rights reserved.

// XMLHttpRequest.js 1.0.3
...
var _lbapi_lb3id="";            // lb3id automatically assigned server-side
...
function liveballData(dname,dvalue,func) {
    if (dname != null && dname != "" && dvalue != null && dvalue != "") {
        var xhr=new XMLHttpRequest;
        xhr.open("GET","/Outside/Data.ashx"+_lbapi_idstg()+"&"+dname+"="+_lbapi_urlencode(dvalue),_lbapi_asynchronous);
        xhr.onreadystatechange=function() {
            if (this.readyState == XMLHttpRequest.DONE && func != null) {
                func();
            }
